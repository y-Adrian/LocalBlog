...
- 内核再映射为全局 **Linux IRQ 号**（`virq`）。驱动通常先通过 `platform_get_irq()` / `of_irq_get()` 取得 `virq`，再传给 `request_irq()`；不要把 GIC 手册里的 SPI 编号直接当作 `request_irq()` 参数（多数平台有 **irq_domain** 做映射）。
...
| **边沿** | 跳变触发一次，控制器通常只记录“发生过” | 处理慢或 mask 期间多次跳变可能合并成一次；需要读设备状态兜底 |
...
3. softirq 运行时仍可能被更高优先级硬中断打断；它的关键限制是 **自己不可睡眠**，且不能假设拥有进程上下文。
