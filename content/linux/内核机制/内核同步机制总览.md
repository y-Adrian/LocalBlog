...
与 **优先级反转（priority inversion）** 相关的 **优先级继承（PI，priority inheritance）** 见 [[linux/内核机制/进程调度与绑核#优先级反转（priority inversion）与对策]]。
...
- **发布新指针不必等读者**；**`kfree` 旧对象必须等宽限期**（见 [[linux/内核机制/RCU 读拷贝更新机制详解]]）。
