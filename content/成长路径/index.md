...
- 优先级：**高** → **中** → **低**（同模块内已标注）。
- 学习时 **零散疑问** 记在 [[学习疑问/疑问记录]]（一行一条），由 Agent 补文后回链到 **已解**。
...
- [x] **进程调度**：CFS、nice、`SCHED_FIFO`；面试详解 → [[linux/内核机制/Linux 内核调度机制面试详解]]；绑核实践 → [[linux/内核机制/进程调度与绑核]]
...
### 6.3 RISC-V 板级实践日志（进行中）

> 按日在 [[linux/驱动与模块/riscv-驱动开发日志/index|RISC-V 驱动开发日志]] 追加；教程型文章仍写在 `驱动与模块/` 或 `学习路径/`。

- [x] **板卡与环境** 登记（内核版本、triplet、串口）→ [[linux/驱动与模块/riscv-驱动开发日志/index|日志索引 §1]]、[[linux/驱动与模块/riscv-驱动开发日志/2026-05-30 — 环境搭建]]
...
> 与嵌入式 Linux + DPDK 方向对齐；**C 打底 → 边界与 UB → C++ 对象模型 → 测对 → 写快**。

### 8.5.1 已有（✅）

- [x] **C 内存模型与 UB** → [[编程语言/C/C 内存模型与未定义行为]]
- [x] **C 编译 / 链接 / ABI** → [[编程语言/C/C 编译链接与 ABI]]
- [x] **C 与 C++ 混用**（`extern "C"`、所有权）→ [[编程语言/C/C 与 C++ 混用]]
- [x] **C++ 对象模型与 Rule of 0/3/5** → [[编程语言/C++/C++ 对象模型与 Rule of Zero-Three-Five]]
- [x] **Sanitizer + 单元测试** → [[编程语言/C/C++ Sanitizer 与单元测试入门]]
- [x] **C/C++ 性能优化方法论** → [[编程语言/C/C++ 性能优化方法论]]
- [x] **C 字符串与 POSIX I/O** → [[编程语言/C/C 字符串与 POSIX I/O 精读]]
- [x] **C99/C11 实用特性** → [[编程语言/C/C99-C11 实用特性]]
- [x] **C++ ABI 深读** → [[编程语言/C++/C++ ABI 深读]]
- [x] **PMR / 自定义分配器** → [[编程语言/C++/PMR 与自定义分配器]]
- [x] **MISRA / CERT C 规范对照** → [[工程基础/MISRA C 与 CERT C 编码规范对照]]
- [x] **C++20 协程与 Modules 选用** → [[编程语言/C++/C++20 协程与 Modules 选用决策]]
- [x] **阅读 C/C++ 源码** → [[编程语言/C/阅读 C 与 C++ 源码的方法]]
- [x] **四阶段实践验收**（echo / extern C / fd RAII / ASan+perf）→ [[编程语言/C/C-C++ 主线实践验收]]

### 8.5.2 待补（⬜）

**优先级：低**

- [x] **MISRA C / CERT C** 子集与评审清单 → [[工程基础/MISRA C 与 CERT C 编码规范对照]]
- [x] **C++20 协程 / Modules** 嵌入式/DPDK 决策 → [[编程语言/C++/C++20 协程与 Modules 选用决策]]

（§8.5 精通 C/C++ 主线 **已全部成文**；后续按项目实践复盘即可。）
...
> **控制面**：共识、选主、复制日志；与 §七 RPC、§十一 数据库复制衔接，**不**与 DPDK 数据面混目录。

### 11.5.1 已有（✅）

- [x] **目录与总览** → [[分布式系统/index]]、[[分布式系统/共识算法/分布式共识算法概览|分布式共识算法概览]]
- [x] **Paxos 与 Multi-Paxos 速览** → [[分布式系统/共识算法/Paxos 与 Multi-Paxos 速览]]
- [x] **Raft 原理与工程实现**（选举、日志匹配、快照、成员变更）→ [[分布式系统/共识算法/Raft 原理与工程实现]]
- [x] **共识与复制边界**（对照 PostgreSQL WAL）→ [[分布式系统/共识算法/共识与复制边界]]
- [x] **etcd 与 Raft 案例**（控制面元数据、Watch、租约、K8s）→ [[分布式系统/共识算法/etcd 与 Raft 案例]]
- [x] **玩具 Raft / lab**（3 节点）→ [[分布式系统/共识算法/玩具 Raft 三节点实验]]

### 11.5.2 待补（⬜）

**优先级：低**

- [x] **玩具 Raft / lab**（3 节点最小实验）→ [[分布式系统/共识算法/玩具 Raft 三节点实验]]（代码：`labs/raft-toy/`）
...
| 文件系统 / 存储 | `linux/文件系统/` |
| 驱动案例 | `linux/驱动与模块/` |
| RISC-V 驱动 **开发日志**（流水） | `linux/驱动与模块/riscv-驱动开发日志/` |
| 内核调度 / 网络栈 / eBPF | `linux/内核机制/` |
| DPDK 项目 / AF_XDP | `网络与DPDK/教程/` 或 [[网络与DPDK/实践/index]] |
| 分布式共识 / 选主 / 复制日志 | `分布式系统/共识算法/` |
| perf / 排障 SOP | `系统调试/` |
| 精通 C/C++ 主线 | [[编程语言/精通 C-C++ 学习路径|精通 C/C++ 学习路径]] |
| C++ 与 DPDK 结合 | `编程语言/C++/` |
| C 语言 / C-C++ 混用 / 精通路径 | `编程语言/C/` |
| 清单本身 | `成长路径/`（本页） |
...
| RISC-V 驱动开发日志 | [[linux/驱动与模块/riscv-驱动开发日志/index]] |
| DPDK 从零 | [[网络与DPDK/教程/index]] |
| 分布式共识 | [[分布式系统/共识算法/index]] |
| C++ 并发底层 | [[编程语言/C++/内存模型]] → [[编程语言/C++/无锁编程]] |
| 精通 C/C++ 主线 | [[编程语言/精通 C-C++ 学习路径]] |
| Linux 内核调度（面试） | [[linux/内核机制/Linux 内核调度机制面试详解]] |
| 命令行与构建 | [[工程基础/index]] |
