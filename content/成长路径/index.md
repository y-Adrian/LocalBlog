...
- [ ] **板卡与环境** 登记（内核版本、triplet、串口）→ 复制 [[linux/驱动与模块/riscv-驱动开发日志/日志模板|日志模板]] 写首篇
- [ ] **字符设备 hello** 模块 `insmod` + `/dev` 或 sysfs 验证
- [ ] **设备树** 节点与 `compatible` / `probe` 打通（对照 [[linux/学习路径/设备树实战指南]]）
